...
. Sur la page *clusters*, cliquez sur *Configuration de la hiérarchisation de base* pour le cluster.
+
...
Vous pouvez télécharger un rapport de la page volumes de Tier pour consulter l'état de Tiering de tous les volumes des clusters que vous gérez. Cliquez simplement sur le image:button_download.png["Télécharger"] bouton. Cloud Tiering génère un fichier .CSV que vous pouvez vérifier et envoyer à d'autres groupes selon vos besoins. Le fichier .CSV contient jusqu'à 10,000 lignes de données.
...
. Dans la page Paramètres avancés, cliquez sur l'icône de menu de l'agrégat et sélectionnez *Modifier l'agrégat*.
+
...
.Étapes
. Dans le menu de navigation de gauche, sélectionnez *mobilité > hiérarchisation*.
. Sur la page *clusters*, cliquez sur l'icône de menu d'un cluster et sélectionnez *Infos cluster*.
...
.Étapes
. Dans Cloud Tiering, cliquez sur l'onglet *clusters*.
...
. Cliquez sur *Ajouter un cluster* pour le cluster que vous souhaitez gérer via BlueXP et implémenter le Tiering des données.
